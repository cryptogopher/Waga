...
dframe = data.frame(x = days$"Day"[1:length(fat$"Fat\n[kg]")], y = fat$"Fat\n[kg]")

linmod = lm(y~x, data=dframe)
...
segmod2 = segmented.lm(linmod, seg.Z=~x, psi=list(x=c(60,120,184,240,302)))
# najlepsze trafienie: 66 104 251 275 297
# slope: -.0695 -.0087 -.0312 -.0892 .0230 -.0789

ymin = floor(min(dframe$y))
ysteps = (24 - ymin)%/%2 + 1
yrange = c(24 - 2*ysteps, 24)
plot(segmod2, xlab="Day", ylab="Fat [kg]")
lines(segmod2)
lines(dframe)
